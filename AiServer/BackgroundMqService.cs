        ...
        while (!cts.IsCancellationRequested)
        {
            foreach (var item in queue.GetConsumingEnumerable(cts.Token))
            {
                try
                {
                    ...
                    if (Log.IsDebugEnabled)
                        Log.Debug($"MQ {completed} [{QueueName}] ProcessMessage(): {item.Id}");
                ...
                catch (Exception ex)
                {
                    ...
                    Log.Error($"MQ {failed} [{QueueName}] failed to ProcessMessage with id {item.Id}: {ex.Message}", ex);
                }
            }
        }
